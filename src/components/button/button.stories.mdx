...
* Buttons are used to open or close (toggle) [Modals](/?path=/story/pattern-library-containers-modal--page). 
...
* Focus and hover states: Buttons fade to medium gray (Primary 500) with white text over 500ms.
* **Spacing**: 
  * There is 16/32px mobile/desktop between buttons.
  * There is 32/40px mobile/desktop of space between a button group and any element above it.
* **Accessibility**: 
  * Icons have accessible names: `<svg role="img" aria-label="Close">`. 
  * When users `prefers-reduced-motion`, the button does not fade on `:hover` or `:focus`, but rather changes instantly.
* **Animation**: Buttons can fade in. Opacity changes from 0 to 1 at the same speed as the user scrolls (scrub). Each `<button>` is given the class `.fade-in`: `<button class="btn fade-in">` (*not* indicated in code example below). See [Animation](/?path=/docs/style-guide-animation--page).
...
<Canvas>
  <Story name="Text Button">
    {{
      components: { Button },
      template: `
      <dlp-button type="button" @click="showModal('AgentModal', $event)" class="inline-block">Read Full Bio</dlp-button>
      `
    }}
  </Story>
  <Story name="Icon Button">
    {{
      components: { Button },
      template: `
      <dlp-button @click.stop="close" :icon='true' class="inline-block align-middle">
...
<Source language='scss' code={dedent`
    ...
    &:active, &:hover, &:focus { 
      span, svg {
        ...
        @media (prefers-reduced-motion) {
          @apply transition-none;  /* transition-property: none; */
        }
      }
    }
    &:active, &:hover { 
      ...
      @apply transition-all;     /* transition-property: all; */
      @apply duration-700;       /* transition-duration: 700ms; */
      @media (prefers-reduced-motion) {
        @apply transition-none;  /* transition-property: none; */
      }
    }
  ...
  .btn__icon {
    ...
    svg {
      @apply w-4;              /* width: 16px; */
      @apply h-4;              /* height: 16px; */
    }
  }
  /* Button: styles */
  .btn--style-dark {
